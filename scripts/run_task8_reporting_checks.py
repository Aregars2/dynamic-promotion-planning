...
def specs(include_robustness: bool = True):
    a=load_pickle(ROOT/'artifacts/policy'/VERSION/'policy_optimization.pkl'); r=load_pickle(ROOT/'artifacts/policy'/VERSION/'task7_robustness_runs.pkl')
    base=pd.read_pickle(ROOT/'artifacts/calibration/empirical_bayes/product_behavioral_bootstrap.pkl');pool=pd.read_pickle(ROOT/'artifacts/calibration/empirical_bayes/pooled_behavioral_draws.pkl')
...
def main(include_robustness: bool = True):
    ...
    summary=[]
    transitions=pd.read_csv(ROOT/'results'/VERSION/'tables/task7_robustness_comparison.csv') if include_robustness else None
    ...
    out=ROOT/'results'/VERSION/'tables';out.mkdir(parents=True, exist_ok=True);grid.to_csv(out/'task8_three_policy_reporting_grid.csv',index=False);pd.DataFrame(summary).to_csv(out/'task8_development_summary.csv',index=False);tail.to_csv(out/'task8_main_terminal_tail_full_grid.csv',index=False)
